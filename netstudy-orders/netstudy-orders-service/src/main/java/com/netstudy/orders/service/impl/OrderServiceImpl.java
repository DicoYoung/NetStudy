package com.netstudy.orders.service.impl;
...
import com.netstudy.base.utils.IdWorkerUtils;
import com.netstudy.base.utils.QRCodeUtil;
...
import com.netstudy.orders.config.PayNotifyConfig;
import com.netstudy.orders.mapper.XcOrdersGoodsMapper;
import com.netstudy.orders.mapper.XcOrdersMapper;
import com.netstudy.orders.mapper.XcPayRecordMapper;
...
import com.netstudy.orders.model.po.XcOrders;
import com.netstudy.orders.model.po.XcOrdersGoods;
import com.netstudy.orders.model.po.XcPayRecord;
import com.netstudy.orders.service.OrderService;
...
public class OrderServiceImpl implements OrderService {
    ...
    public PayRecordDto createOrder(String userId, AddOrderDto addOrderDto) {
        ...
        try {
            ...
            NetStudyException.cast("生成二维码出错");
        }
    ...
    public PayStatusDto queryPayResultFromAlipay(String payNo) {
        ...
        try {
            ...
            if (!response.isSuccess()) {
                NetStudyException.cast("请求支付查询查询失败");
            }
    ...
    public void saveAlipayStatus(PayStatusDto payStatusDto) {
        ...
        if (payRecord == null) {
            NetStudyException.cast("未找到支付记录");
        }
        XcOrders order = xcOrdersMapper.selectById(payRecord.getOrderId());
        if (order == null) {
            NetStudyException.cast("找不到相关联的订单");
        }
        ...
        if ("TRADE_SUCCESS".equals(tradeStatus)) {
            ...
            if (updateRecord <= 0) {
                NetStudyException.cast("更新支付交易表失败");
            }
            // 更新订单表
            ...
            if (updateOrder <= 0) {
                log.debug("更新订单表失败");
                NetStudyException.cast("更新订单表失败");
            }
        }
    ...
    public XcPayRecord createPayRecord(XcOrders orders) {
        ...
        if (insert <= 0) {
            NetStudyException.cast("插入支付交易记录失败");
        }
        return payRecord;
    }
